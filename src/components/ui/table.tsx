...
export const TableWithFilters = () => {
  ...
  const filteredData = useMemo(() => {
    return data
      .filter((item) => item.make.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <div className="p-4 bg-white rounded-xl">
      ...
      <div className="flex flex-wrap justify-between  gap-4 mb-5 items-center">
        ...
        >
          <option className="text-[13px]" value={0}>
            All manufacturers
          </option>
          <option className="text-[13px]" value={2}>
            At least 2 models
          </option>
          <option className="text-[13px]" value={5}>
            At least 5 models
          </option>
        </select>
      </div>
